...
# Load your data
df = pd.read_csv("hwc_3d_data.csv")  # Ensure 'S_NAME', 'S_RA', 'S_DEC', etc., columns are present
...
# Function to generate orbit and apply inclination using a rotation matrix
...
# Function to generate sphere for habitable zone
...
# Group data by 'S_NAME' (star system)
def generate_plots(df):
    star_systems = df.groupby('S_NAME')
    plots_html = ""

# Loop through each star system and generate a plot
# Loop through each star system and generate a plot
for star_name, star_data in star_systems:
    ...
    # Simulate orbits for each planet in the star system, relative to the star's position
    orbit_traces = []
    planet_traces = []
    ...
    # Set up the layout with black background and hide the axes
    layout = go.Layout(
    ...
    # Plot the figure for the current star system
